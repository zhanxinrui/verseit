...
//
//import com.screwmachine55open.verseit.dao.PoemDaoImpl;
...
//        Query query = new Query();
//        query.addCriteria(poemDaoImpl.getStyle(styleList))
//                .with(poemDaoImpl.getSort(sortList)); //得到所有需要关注的用户id增加为criteria
...
//        ArrayList<String> sortList = param.get("sort");
//        Query query = new Query();
//        query.addCriteria(poemDaoImpl.getAllConcernedUsersWithGroup(userId,groupList))
//                .addCriteria(poemDaoImpl.getStyle(styleList))
//                .with(poemDaoImpl.getSort(sortList)); //得到所有需要关注的用户id增加为criteria
//        List<Poem> resultList= mongoTemplate.find(query, Poem.class);
//        return resultList;
//    }
